...
const MyTableProperty = () => {
  ...
  const handleDelete = async (e)=>{
    ...
    let confirmar = confirm('Estas seguro que deseas eliminar la propiedad?')
    if(confirmar){
      await axios.delete(`http://127.0.0.1:4000/api/properties/${id}`,{
      headers: {
  ...
  const getProperties = async ()=>{
    const data = (await axios.get(`http://127.0.0.1:4000/api/properties`,{
      headers: {
        'Authorization': getLocalStorage('user').token
      }
    }
